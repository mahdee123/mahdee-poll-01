...
import LockerHistoryPage from './LockerHistoryPage';
...
export default function LockerManagementPage({ token }) {
  ...
  return (
    <div className="grid gap-6">
      ...
      {activeTab === 'dashboard' && (
        <LockerDashboard key={refreshKey} token={token} onSettingsUpdated={() => setRefreshKey((prev) => prev + 1)} />
      )}
      {activeTab === 'history' && <LockerHistoryPage token={token} />}
